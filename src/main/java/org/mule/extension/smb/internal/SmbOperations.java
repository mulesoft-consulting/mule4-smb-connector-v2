...
public final class SmbOperations extends BaseFileSystemOperations {
	...
	public PagingProvider<SmbFileSystem, Result<Object, SmbFileAttributes>> list(@Config FileConnectorConfig config,
																				   @Path(type = DIRECTORY,
	...
	@MediaType(value = ANY, strict = false)
	public Result<InputStream, SmbFileAttributes> read(@Config FileConnectorConfig config,
														@Connection SmbFileSystem fileSystem,
		...
		Result result =
				doRead(config, fileSystem, path, lock,
						config.getTimeBetweenSizeCheckInMillis(timeBetweenSizeCheck, timeBetweenSizeCheckUnit).orElse(null));
		return (Result<InputStream, SmbFileAttributes>) result;
	}
	 ...
	 * value of the {@code createParentFolder} argument. If such argument is {@false}, then a {@code SMB:ILLEGAL_PATH} will
	 * be thrown.
	 * <p>
